...
6. 서버로부터 토큰이 만료되었다는 응답을 받으면, 클라이언트는 `Access 토큰 재발급 API` 을 Refresh 토큰과 함께 요청한다.
7. 서버는 DB에 저장된 Access Token이 실제로 만료되었고, Refresh 토큰이 유효하다면, 새로운 Access Token으로 DB를 업데이트하고 응답한다.
...
해커가 Refresh 토큰을 탈취하여, `Access 토큰 재발급 API` 를 호출한다면 새로운 Access 토큰을 통해 악의적인 작업을 할 수 있지 않을까?

결론적으로 그렇지 않다.

1. 해커가 탈취한 Refresh 토큰으로 새로운 Access 토큰을 재발급 받고자, `Access 토큰 재발급 API` 를 호출한다.
2. 서버는 해당 Refresh 토큰과 1대1로 매핑된 Access 토큰이 실제로 만료되었는지 확인한다. (DB를 통해)
3. **하지만 Access 토큰이 실제로 만료되지 않았기 때문에, 비정상적인 접근으로 판단하여 해당 Refresh 토큰과 Access 토큰을 모두 제거한다.**
...
1. 해커가 탈취한 Refresh 토큰으로 새로운 Access 토큰을 재발급 받고자, `Access 토큰 재발급 API` 를 호출한다.
2. 서버는 해당 Refresh 토큰과 1대1로 매핑된 Access 토큰이 실제로 만료되었는지 확인한다. (DB를 통해)
3. 실제로 Access 토큰이 만료되어, 해커에게 새로운 Access 토큰을 발급한다.
4. **정상 사용자가 만료된 Access 토큰을 서버에 전달하면, 서버가 만료된 토큰임을 알려준다.**
5. **정상 사용자가 Refresh 토큰으로 새로운 Access 토큰을 재발급 받고자, `Access 토큰 재발급 API` 를 호출한다.**
6. 서버는 해당 Refresh 토큰과 1대1로 매핑된 Access 토큰이 실제로 만료되었는지 확인한다. (DB를 통해)
7. **하지만 해커에게 발급한 Access 토큰이 만료되지 않았기 때문에, 비정상적인 접근으로 판단하여 해당 Refresh 토큰과 Access 토큰을 모두 제거한다.**
8. 결과적으로 해커와 정상 사용자 모두 다시 로그인해야한다.

위처럼 해커가 새로운 Access Token을 발급받는데 성공하긴 하지만, 정상 사용자가 다시 Access Token을 발급받으면 비정상 접근으로 판단할 수 있다.
