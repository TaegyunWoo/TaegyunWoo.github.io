...
- `@IdClass(A.class)` 애너테이션의 의미
    - **해당 애너테이션이 붙은 엔티티 클래스의 각 속성(필드) 중, `@Id` 가 붙은 속성(필드)를 `A` 식별자 클래스의 속성(필드)와 매핑시키겠다는 의미이다.**
...
- **자식 클래스**
    ...
    - **비식별 관계인 경우, `parent` 필드를 선언할 필요가 없다.**
        - 왜냐하면 식별자 클래스를 통해, 매핑 설정을 할 수 있기 때문이다.
    - **하지만 식별 관계인 경우, `parent` 필드를 선언해야 한다.**
        - 왜냐하면 식별자 클래스에서 '매핑 설정과 함께 연관관계를 설정'할수는 없기 때문에, `Child` 클래스에서 연관관계를 설정해야하기 때문이다.
    - `@MapsId("parentId")`
        - 식별자 클래스의 `parentId` 필드에 아래 연관관계를 적용하겠다는 의미와 같다.
            - `@ManyToOne` , `@JoinColumn`
        - PK 설정 ⇒ 식별자 클래스에서 설정
        - FK 설정 ⇒ 엔티티 클래스에서 설정 ⇒ 식별자 클래스에게도 전달해줌
...
- 상세 설명
    ...
    - `@IdClass` 와의 차이점
        ...
        - `@EmbeddedId` 는 엔티티의 연관관계 속성(필드)에 `@MapsId` 를 사용한다.
            - 참고로 위에서 설명했듯이, 만약 비식별 관계였다면 애초에 엔티티 클래스에서 해당 속성(필드)를 선언할 필요조차 없다.
    - `@MapsId` 의 의미
